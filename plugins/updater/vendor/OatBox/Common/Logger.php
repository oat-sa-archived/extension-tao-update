...
class Logger
{
    ...
    private function __construct()
    {
        ...
        $this->implementor = Log\Dispatcher::singleton();
    ...
    public function handleException(Exception $exception)
    {
        ...
        self::singleton()->log($severity, $exception->getMessage(), array(
            get_class($exception)
        ...
        if (error_reporting() != 0) {
            ...
            switch ($errorNumber) {
                ...
                case E_STRICT:
                    ...
                    break;
                default:
                    self::d('Unsuported PHP error type: ' . $errorNumber, 'Logger');
                    $severity = self::ERROR_LEVEL;
                    break;
            }
            self::singleton()->log($severity, 'php error(' . $errorNumber . '): ' . $errorString, array(
                'PHPERROR'
            ), $errorFile, $errorLine);
        }
    ...
    public function handlePHPShutdown()
    {
        ...
        if (($error['type'] & (E_COMPILE_ERROR | E_ERROR | E_PARSE | E_CORE_ERROR)) != 0) {
            if (isset($error['file']) && isset($error['line'])) {
                self::singleton()->log(self::FATAL_LEVEL, 'php error(' . $error['type'] . '): ' . $error['message'], array(
                    'PHPERROR'
